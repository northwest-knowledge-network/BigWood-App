...
lapply(c("shinythemes", "shiny", "RPostgres", "DBI", "ggplot2", "stringr", "leaflet", "sf", "shinyWidgets", "dplyr", "tidyr"), require, ch = T)
...
server <- function(input, output) {
  ...
  output$forecasted_vols <- renderTable(dbGetQuery(conn, 'SELECT "Exceedance", "bwh.irr_vol" AS "Big Wood at Hailey", "bws.irr_vol" AS "Big Wood at Stanton", "sc.irr_vol" AS "Silver Creek", "cc.irr_vol" AS "Camas Creek" FROM exceednaceprobabilities;'),
    width = "90%", digits = 0
  ) # note table misspelling, need to fix
  ...
  tf_airTemps <- reactive({
    getAirTempsByDate(input$tf_date)
  })
  tf_maxAirTemps <- reactive({
    getAirTempsByDate(input$tf_date, high = T)
  })

  output$tf_airTempDisplay <- renderText({
    paste0(
      "Daily average average air temperatures at the Picabo AgriMet station for ",
  ...
  tf_indexFlows <- reactive({
    getIndexFlowsByDate(input$tf_date)
  })
  tf_predFlows <- reactive({
    getPredictedFlowsByDate(input$tf_date)
  })

  tf_stats <- reactive({
    getTemperatureStats(tf_indexFlows = tf_indexFlows(), tf_predFlows = tf_predFlows(), airTemp = quantile(tf_airTemps(), .9), forecastDate = tf_date(), streamSegs = streamSegs, temperatureModel = temperatureModel)
  })

  output$tf_flowDisplay <- renderText({
    paste0(
      "Streamflow in Silver Creek is described in terms of the flow at the Sportsmans Gauge.  The predicted streamflow at Sportsmans for ",
  ...
  temperatureMap <- addWMSTiles(
    map = temperatureMap, baseUrl = "https://basemap.nationalmap.gov/arcgis/services/USGSImageryOnly/MapServer/WmsServer",
    layers = 0,
  ...
  dataExtentMap <- addWMSTiles(
    map = dataExtentMap, baseUrl = "https://basemap.nationalmap.gov/arcgis/services/USGSImageryOnly/MapServer/WmsServer",
    layers = 0,
    attribution = 'Tiles courtesy of the <a href="https://usgs.gov/">U.S. Geological Survey</a>',
    tileOptions(zIndex = 1)
  )
  ...
  plotData <- reactive(getDataByVarTimeExtent(
    useVars = input$plotVars,
  ...
  observeEvent(input$makePlot, {
    plotData <- getDataByVarTimeExtent(
      useVars = input$plotVars,
      startDateTime = input$plotDateRange[1],
      endDateTime = input$plotDateRange[2],
      extent = input$plotExtent_bounds
  ...
  observe({
    locationPoints <- getLocationsForVariables(
      useVars = input$plotVars,
